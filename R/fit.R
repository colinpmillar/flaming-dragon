...
ind = (YPR.data$WG==WG)&(YPR.data$stock==SN)

matp = YPR.data$Mat[ind] 
mp = YPR.data$M[ind]
age = YPR.data$age[ind]
n.age = length(age)
stock.weight = YPR.data$WeSt[ind] 
catch.weight = YPR.data$WeCa[ind]
Sel = YPR.data$F[ind]
pr = Sel/max(Sel)

ind = (limits.data$WG==WG)&(limits.data$stock==SN)
ices.Fmsy = limits.data$FMSY[ind]

########################################################;

ind = (stock.names$WG==WG)&(stock.names$stock==SN)
stock.name <-stock.names$name[ind] 
ind = (age.data$WG==WG)&(age.data$stock==SN)
rec.age = age.data$rec.age[ind]
ind = (sr.data$WG==WG)&(sr.data$stock==SN)
sscale = 1000
s.name = c('SSB (Kt)') 
rscale = 1000
...
n = length(sr.data$year[ind])
s <- sr.data$s[ind][1:(n-rec.age)]
r <- sr.data$r[ind][(rec.age+1):n]
year = sr.data$year[ind][(rec.age+1):n]

ind1 = !is.na(r)
r = r[ind1];s = s[ind1];year=year[ind1];   
ind1 = !is.na(s)
r = r[ind1];s = s[ind1];year=year[ind1];
...
sr_type='BH'
bh.ref.points = make.RP(log(BH.parm),0.2)
BH.Bmsy = bh.ref.points[4]

BH.boo.RP = apply(log(BH.boo$coef),1,make.RP,Fstart=0.1);
...
sr_type<<-'HS'
hs.ref.points = make.RP(log(HS.parm),0)
HS.Bmsy = hs.ref.points[4]

HS.boo.RP = apply(log(HS.boo$coefboot),1,make.RP,Fstart=0);
...
RK.fit <- nls(logrec ~ log(alpha) + lbeta + one + logssb - exp(lbeta)*ssb,
           ...
           algorithm="port",lower=c(0,-Inf),upper=c(Rmax,0))
