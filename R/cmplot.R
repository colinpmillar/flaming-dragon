...
do.line <- function(g, s, link = "log", ...) {

  fun <- if (link == "log") exp else function(x) 1/x
  fit <- predict(g, newdata = list(s=s), se = TRUE)
  
  lines(s, fun(fit $ fit), ...)
  lines(s, fun(fit $ fit + 2*fit $ se), ...)
  lines(s, fun(fit $ fit - 2*fit $ se), ...)
}


test <- function(bs) {
  g1 <- glm( r ~ s + offset(log(s)), family = Gamma(log))
  g2 <- glm( r ~ I(1/s), family = Gamma(inverse))

  g3 <- scam( r ~ s(s, bs = bs) + offset(log(s)), family = Gamma(log))
  g4 <- scam( r ~ s(I(1/s), bs = bs), family = Gamma(inverse))

  plot(s, r, xlim = c(0,max(s) * 3), ylim=c(0,max(r)))
  s. <- seq(0.1, max(s)*3, length = 100)

  do.line(g1, s., "log", col = "blue", lty = 2)
  do.line(g2, s., "inverse", col = "red", lty = 2)

  do.line(g3, s., "log", col = "blue", lwd = 2)
  do.line(g4, s., "inverse", col = "red", lwd = 2)
}
...
file.name <- paste0('fig/fit',c,'.#pdf') 
#pdf(file=file.name) 
...
for(i in 1:nFv){
  ...
  temp = make.RP(log(BH.parm),F,0, sr_type='BH')
  if(temp[4]<ssb.max.bh){BH.equil[i,] = temp[3:5]} 
  sr_type='RK'
  temp = make.RP(log(RK.parmi),F,0, sr_type='RK')
  if(temp[4]<ssb.max){RK.equil[i,] = temp[3:5]} 
  sr_type='HS'
  temp = make.RP(log(HS.parm),F,0, sr_type='HS')
...
CM.boo.qRP = 
  apply(CM.boo.RP,1,quantile,probs=c(0.025,0.25,0.5,0.75,0.975),na.rm=T)
colnames(CM.boo.qRP) = c('Fmsy','Bmsy','MSY')
CM3.boo.qRP = 
  apply(CM3.boo.RP,1,quantile,probs=c(0.025,0.25,0.5,0.75,0.975),na.rm=T)
colnames(CM3.boo.qRP) = c('Fmsy','Bmsy','MSY')
CM4.boo.qRP = 
  apply(CM4.boo.RP,1,quantile,probs=c(0.025,0.25,0.5,0.75,0.975),na.rm=T)
